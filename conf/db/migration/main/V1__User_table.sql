...
BEGIN
  RETURN QUERY
  SELECT id, hostid, date, start, "end", appointmentduration, place, TRUE FROM "CustomSchedule"
  WHERE date >= p_from AND date < p_to AND hostid = p_hostid
  UNION
  SELECT g_id, g_hostId, g_date, g_start, g_end, g_appointmentDuration, g_place, FALSE FROM generate_schedule(p_hostid, p_from, p_to) as GEN
